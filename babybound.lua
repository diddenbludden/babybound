...
local function LoadAvatarOntoCharacter(userId)
    ...
    task.spawn(function()
        pcall(function()
            -- Fetch avatar description for the target user
            local humanoidDesc = Players:GetHumanoidDescriptionFromUserId(uid)
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum and humanoidDesc then
                hum:ApplyDescription(humanoidDesc)
                Rayfield:Notify({ Title="Avatar", Content="Avatar applied!", Duration=3, Image=4483362458 })
            else
                Rayfield:Notify({ Title="Avatar", Content="Failed to apply description.", Duration=3, Image=4483362458 })
            end
        end)
    end)
end
...
local function ResetAvatar()
    task.spawn(function()
        pcall(function()
            ...
            Rayfield:Notify({ Title="Avatar", Content="Avatar reset to your original.", Duration=3, Image=4483362458 })
        end)
    end)
end

-- Sandbox: load any user ID avatar into a cloned dummy you can inspect
...
local function SpawnSandboxDummy(userId)
    ...
    task.spawn(function()
        pcall(function()
            ...
            -- Create a basic R15 dummy rig
            local dummy = Instance.new("Model")
            dummy.Name = "AvatarSandbox_" .. tostring(uid)
            -- Use InsertService to get a base R15 character
            local InsertService = game:GetService("InsertService")
            local baseModel = InsertService:LoadAsset(104508819) -- R15 base rig asset
    ...
    task.spawn(function()
        ...
        if hum then
            ...
            if ok2 then
                ...
                Rayfield:Notify({ Title="Avatar", Content="Apply failed: "..tostring(err), Duration=4, Image=4483362458 })
            end
        end
    end)
end
...
local _editorDesc = nil  -- current working HumanoidDescription
...
local function ApplyWorkingDesc()
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    pcall(function() hum:ApplyDescription(_editorDesc) end)
end

local function MakeEditorLabel(parent, text, size, pos, color, fontSize)
...
local function MakeEditorInput(parent, placeholder, size, pos, callback)
    local frame = Instance.new("Frame")
    frame.Size = size
    frame.Position = pos
...
local function MakeEditorButton(parent, text, size, pos, color, callback)
    local btn = Instance.new("TextButton")
    btn.Size = size
    btn.Position = pos
    btn.BackgroundColor3 = color or Color3.fromRGB(60, 60, 75)
...
local function MakeSliderRow(parent, labelText, yPos, minV, maxV, currentV, prop)
    MakeEditorLabel(parent, labelText, UDim2.new(0, 110, 0, 20), UDim2.new(0, 8, 0, yPos), Color3.fromRGB(180,180,180), 12)
    local track = Instance.new("Frame")
    track.Size = UDim2.new(0, 150, 0, 8)
    track.Position = UDim2.new(0, 120, 0, yPos + 6)
    ...
    knob.Position = UDim2.new((currentV - minV)/(maxV - minV), -7, 0.5, -7)
    knob.BackgroundColor3 = Color3.fromRGB(200, 220, 255)
    knob.Text = ""
    knob.BorderSizePixel = 0
    knob.Parent = track
    local kc = Instance.new("UICorner") kc.CornerRadius = UDim.new(1,0) kc.Parent = knob
    local valLabel = MakeEditorLabel(parent, tostring(currentV), UDim2.new(0,40,0,20), UDim2.new(0,278,0,yPos), Color3.fromRGB(150,200,255), 12)
    ...
    game:GetService("RunService").RenderStepped:Connect(function()
        ...
        local rel = math.clamp((mouse.X - trackAbs.X) / trackSize.X, 0, 1)
        local val = math.floor(minV + (maxV - minV) * rel)
        fill.Size = UDim2.new(rel, 0, 1, 0)
        knob.Position = UDim2.new(rel, -7, 0.5, -7)
        valLabel.Text = tostring(val)
        if _editorDesc and _editorDesc[prop] ~= nil then
            pcall(function() _editorDesc[prop] = val end)
        end
    end)
end
...
local function MakeColorRow(parent, labelText, yPos, prop)
    ...
    preview.Position = UDim2.new(0, 120, 0, yPos - 1)
    preview.BorderSizePixel = 0
    preview.Parent = parent
    local pc = Instance.new("UICorner") pc.CornerRadius = UDim.new(0,4) pc.Parent = preview
    if _editorDesc and typeof(_editorDesc[prop]) == "Color3" then
        preview.BackgroundColor3 = _editorDesc[prop]
    else
        preview.BackgroundColor3 = Color3.fromRGB(255,255,255)
    end
    -- R G B inputs
    local function makeChannel(label, xOff, channel)
        MakeEditorLabel(parent, label, UDim2.new(0,10,0,20), UDim2.new(0,150+xOff,0,yPos), Color3.fromRGB(200,200,200), 11)
        local box = MakeEditorInput(parent, "0-255", UDim2.new(0,38,0,20), UDim2.new(0,160+xOff,0,yPos), function(v)
        ...
        if _editorDesc and typeof(_editorDesc[prop]) == "Color3" then
            local cur = _editorDesc[prop]
            if channel == "R" then box.Text = tostring(math.floor(cur.R*255))
            elseif channel == "G" then box.Text = tostring(math.floor(cur.G*255))
            else box.Text = tostring(math.floor(cur.B*255)) end
        end
    end
    makeChannel("R", 0, "R")
    makeChannel("G", 45, "G")
...
local CLOTHING_PROPS = {
    { label="Shirt ID",       prop="Shirt" },
    { label="Pants ID",       prop="Pants" },
    { label="T-Shirt ID",     prop="GraphicTShirt" },
    { label="Face ID",        prop="Face" },
    { label="Head ID",        prop="Head" },
    { label="Back Acc ID",    prop="BackAccessory" },
    { label="Face Acc ID",    prop="FaceAccessory" },
    { label="Front Acc ID",   prop="FrontAccessory" },
    { label="Hat/Hair Acc",   prop="HatAccessory" },
    { label="Neck Acc ID",    prop="NeckAccessory" },
    { label="Shoulder Acc",   prop="ShouldersAccessory" },
    { label="Waist Acc ID",   prop="WaistAccessory" },
}

local SCALE_PROPS = {
    { label="Height Scale",     prop="HeightScale",     min=0.9,  max=1.05, step=0.01 },
    { label="Width Scale",      prop="WidthScale",      min=0.7,  max=1.0,  step=0.01 },
    { label="Head Scale",       prop="HeadScale",       min=0.95, max=1.0,  step=0.01 },
    { label="Body Depth",       prop="DepthScale",      min=0.7,  max=1.0,  step=0.01 },
    { label="Proportion",       prop="ProportionScale", min=0,    max=1,    step=0.01 },
}

local BODY_COLOR_PROPS = {
    { label="Head Color",       prop="HeadColor" },
    { label="Torso Color",      prop="TorsoColor" },
    { label="Left Arm Color",   prop="LeftArmColor" },
    { label="Right Arm Color",  prop="RightArmColor" },
    { label="Left Leg Color",   prop="LeftLegColor" },
    { label="Right Leg Color",  prop="RightLegColor" },
}
...
local function OpenAvatarEditor()
    ...
    _editorGui.Name = "BabyBoundAvatarEditor"
    ...
    MakeEditorLabel(titleBar, "🎨  Avatar Editor", UDim2.new(1,-80,1,0), UDim2.new(0,12,0,0), Color3.fromRGB(200,180,255), 15)
    MakeEditorButton(titleBar, "✕", UDim2.new(0,28,0,28), UDim2.new(1,-36,0,4), Color3.fromRGB(180,50,50), function()
        _editorGui.Enabled = false
    end)
    ...
    local function SwitchTab(idx)
        for i, f in pairs(tabFrames) do
            f.Visible = (i == idx)
            tabBtns[i].BackgroundColor3 = (i == idx) and Color3.fromRGB(70,60,100) or Color3.fromRGB(35,35,45)
        end
    end
    ...
    contentArea.ClipDescendants = true
    contentArea.Parent = win

    for i, name in ipairs(TAB_NAMES) do
        local btn = MakeEditorButton(tabBar, name, UDim2.new(tabW, -2, 1, -6), UDim2.new((i-1)*tabW, 1, 0, 3),
            i == 1 and Color3.fromRGB(70,60,100) or Color3.fromRGB(35,35,45), function() SwitchTab(i) end)
        tabBtns[i] = btn
    ...
    for _, item in ipairs(CLOTHING_PROPS) do
        MakeEditorLabel(clothFrame, item.label, UDim2.new(0,110,0,20), UDim2.new(0,8,0,cy), Color3.fromRGB(180,180,180), 12)
        local currentVal = ""
        pcall(function()
        ...
        local box = MakeEditorInput(clothFrame, "Asset ID", UDim2.new(0,180,0,22), UDim2.new(0,120,0,cy), function(v)
            local num = tonumber(v)
            if num and _editorDesc then
                pcall(function() _editorDesc[item.prop] = num end)
            end
        end)
        box.Text = currentVal
        cy = cy + 30
    ...
    for _, item in ipairs(SCALE_PROPS) do
        local currentVal = 1
        pcall(function()
            if _editorDesc[item.prop] ~= nil then currentVal = _editorDesc[item.prop] end
        end)
        MakeSliderRow(scaleFrame, item.label, sy, math.floor(item.min*100), math.floor(item.max*100), math.floor(currentVal*100), item.prop)
        -- override: scale props need float division
        -- patch: we store *100 ints, so wrap apply
        sy = sy + 30
    end
    ...
    for _, item in ipairs(BODY_COLOR_PROPS) do
        MakeColorRow(colorFrame, item.label, colY, item.prop)
        colY = colY + 34
    end
    ...
    MakeEditorLabel(importFrame, "Steal avatar from User ID:", UDim2.new(1,-16,0,20), UDim2.new(0,8,0,10), Color3.fromRGB(200,200,200), 13)
    local importBox = MakeEditorInput(importFrame, "Roblox User ID", UDim2.new(1,-16,0,26), UDim2.new(0,8,0,34), function() end)
    MakeEditorButton(importFrame, "Import to Editor", UDim2.new(1,-16,0,30), UDim2.new(0,8,0,68), Color3.fromRGB(60,100,60), function()
    ...
    MakeEditorLabel(importFrame, "Or load your own:", UDim2.new(1,-16,0,20), UDim2.new(0,8,0,110), Color3.fromRGB(200,200,200), 13)
    MakeEditorButton(importFrame, "Load My Current Avatar", UDim2.new(1,-16,0,30), UDim2.new(0,8,0,134), Color3.fromRGB(60,60,100), function()
        _editorDesc = GetCurrentDesc()
        Rayfield:Notify({ Title="Editor", Content="Loaded your current avatar.", Duration=3, Image=4483362458 })
    end)
    importFrame.CanvasSize = UDim2.new(0, 0, 0, 180)
    ...
    local bbc = Instance.new("UICorner") bbc.CornerRadius = UDim.new(0,8) bbc.Parent = bottomBar

    MakeEditorButton(bottomBar, "✅ Apply to Character", UDim2.new(0.5,-4,0,26), UDim2.new(0,4,0,4),
        Color3.fromRGB(50,120,60), function()
            ApplyWorkingDesc()
            Rayfield:Notify({ Title="Avatar Editor", Content="Applied!", Duration=3, Image=4483362458 })
        end)
    MakeEditorButton(bottomBar, "↩ Reset", UDim2.new(0.25,-4,0,26), UDim2.new(0.5,4,0,4),
        Color3.fromRGB(100,60,40), function()
            _editorDesc = GetCurrentDesc()
            Rayfield:Notify({ Title="Avatar Editor", Content="Reset to current avatar.", Duration=3, Image=4483362458 })
        end)
    MakeEditorButton(bottomBar, "🗑 Clear Accs", UDim2.new(0.25,-4,0,26), UDim2.new(0.75,4,0,4),
        Color3.fromRGB(80,40,40), function()
            if _editorDesc then
                pcall(function()
                    _editorDesc.HatAccessory = ""
                    _editorDesc.HairAccessory = ""
                    _editorDesc.FaceAccessory = ""
                    _editorDesc.BackAccessory = ""
                    _editorDesc.WaistAccessory = ""
                    _editorDesc.NeckAccessory = ""
                    _editorDesc.FrontAccessory = ""
                    _editorDesc.ShouldersAccessory = ""
                end)
                Rayfield:Notify({ Title="Avatar Editor", Content="Accessories cleared. Hit Apply.", Duration=3, Image=4483362458 })
            end
        end)
...
AppearanceTab:CreateButton({
    Name = "Apply Avatar",
    Callback = function()
        LoadAvatarOntoCharacter(_avatarTargetId)
    end,
})
AppearanceTab:CreateButton({
